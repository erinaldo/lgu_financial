...
Public Class frmNewDirectJournal
    ...
    Public Sub ViewList()
        LoadXgrid("SELECT periodcode,officeid, pid as 'Entry Code', if(cancelled,'CANCELLED',if(approved,'APPROVED',if(draft,'DRAFT',if(hold,'HOLD',if(forapproval,'FOR APPROVAL','-'))))) as Status, " _
                        ...
                        + " where approved=1 and cancelled=0 and jev=0 and " _
                        + " requesttype in (select code from tblrequisitiontype where enablevoucher=0) and " _
    ...
    Private Sub cmdView_Click(sender As Object, e As EventArgs) Handles cmdView.Click
        ...
        frmJournalEntry.txtRemarks.Text = GridView1.GetFocusedRowCellValue("Purpose").ToString
        frmJournalEntry.officeid.Text = GridView1.GetFocusedRowCellValue("officeid").ToString
        frmJournalEntry.txtOffice.Text = GridView1.GetFocusedRowCellValue("Requesting Office").ToString
        frmJournalEntry.pid.Text = GridView1.GetFocusedRowCellValue("Entry Code").ToString
