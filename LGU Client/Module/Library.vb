...
Module library
    ...
    Public Function FormatDate(ByVal fieldname As String, ByVal formal As Boolean)
        If formal = True Then
            FormatDate = "date_format(" & fieldname & ",'%M %d, %Y %r')"
        Else
            FormatDate = "date_format(" & fieldname & ",'%Y-%m-%d %r')"
        End If
        Return FormatDate
    End Function
    ...
    Public Function GetServerDate() As Date
        com.CommandText = "select current_date as dt"
        rst = com.ExecuteReader
        While rst.Read
            GetServerDate = CDate(rst("dt").ToString)
        End While
        rst.Close()
    ...
    Public Function SearchAdvanceCommand(ByVal fieldname As String, ByVal strString As String)
