...
Module FileManagementModule
    ...
    Public Function ViewAttachmentPackage_All(ByVal refno As Array, ByVal trntype As String)
        Dim Root_location As String = System.IO.Path.GetTempPath() & "Dockeeper\" & globaluser & "\"
        Dim extract_location As String = System.IO.Path.GetTempPath() & "Dockeeper\" & globaluser & "\" & CDate(Now).ToString("yyyyMMddhhmmss") & "\"
    ...
    Public Function DeleteAttachedFiles(ByVal refnumber As String)
        Try
            dst = Nothing : dst = New DataSet
            msda = New MySqlDataAdapter("select * from " & sqlfiledir & ".tblattachmentlogs where refnumber='" & refnumber & "'", conn)
            msda.Fill(dst, 0)
            For cnt = 0 To dst.Tables(0).Rows.Count - 1
                With (dst.Tables(0))
                    com.CommandText = "DELETE FROM " & sqlfiledir & ".`" & .Rows(cnt)("databasename").ToString() & "` where archievedname = '" & .Rows(cnt)("archievedname").ToString() & "' and filesize = '" & .Rows(cnt)("filesize").ToString() & "' and trntype='" & .Rows(cnt)("trntype").ToString() & "' and refnumber='" & .Rows(cnt)("refnumber").ToString() & "'" : com.ExecuteNonQuery()
                End With
            Next
            com.CommandText = "delete from " & sqlfiledir & ".tblattachmentlogs where refnumber='" & refnumber & "'" : com.ExecuteNonQuery()
