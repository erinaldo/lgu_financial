...
Public Class frmBudgetMonthly
    ...
    Public Sub LoadExpenditureItem()
        ...
        LoadXgrid("select *, COALESCE(NULLIF(((ifnull(January,0)+ifnull(February,0)+ifnull(March,0)+ifnull(April,0)+ifnull(May,0)+ifnull(June,0)+ifnull(July,0)+ifnull(August,0)+ifnull(September,0)+ifnull(October,0)+ifnull(November,0)+ifnull(December,0)+ifnull(nydd,0)+ifnull(dd,0)+ifnull(cleared,0)+`Current Month Balance`)-Amount)-`Total Budget`, 0.00), null) as VARIANCE " _
                  ...
                  + " COALESCE(NULLIF((Select ifnull(sum(amount),0) from tblrequisitionfund as x inner join tblrequisition as pr on x.pid=pr.pid where b.periodcode=x.periodcode And b.itemcode=x.itemcode and b.officeid=x.officeid And x.cancelled=0 and pr.paid=0 and pr.cleared=0), 0), null) as 'NYDD', " _
                  + " COALESCE(NULLIF((Select ifnull(sum(amount),0) from tblrequisitionfund as x inner join tblrequisition as pr on x.pid=pr.pid where b.periodcode=x.periodcode And b.itemcode=x.itemcode and b.officeid=x.officeid And x.cancelled=0 and pr.approved=1 and pr.paid=1 and pr.cleared=0), 0), null) as 'DD', " _
                  + " COALESCE(NULLIF((Select ifnull(sum(amount),0) from tblrequisitionfund as x inner join tblrequisition as pr on x.pid=pr.pid where b.periodcode=x.periodcode And b.itemcode=x.itemcode and b.officeid=x.officeid And x.cancelled=0 and pr.approved=1 and pr.cleared=1), 0), null) as 'CLEARED' " _
